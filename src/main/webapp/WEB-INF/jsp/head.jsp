...
<header class="head">
	<div class="inner">
		...
		<nav class="head-nav nav nav-pills nav-fill bg-light">
		    <a class="nav-item nav-link btn-success <%=array[0] %>" href="/">首页</a>
		    <c:if test="${(cookie['identity'].value eq 'user') || (cookie['identity'].value eq 'admin')}">
		    <a class="nav-item nav-link btn-success <%=array[1] %>" href="/basedata/">基本数据管理</a>
		    <a class="nav-item nav-link btn-success <%=array[2] %>" href="/coursearrange/">课程安排管理</a>
		    <a class="nav-item nav-link btn-success <%=array[3] %>" href="/studentscore/">学生成绩管理</a>
		    <a class="nav-item nav-link btn-success <%=array[4] %>" href="/selfinfo/">个人信息管理</a>
		    </c:if>
		    <c:if test="${cookie['identity'].value eq 'student'}">
		    <a class="nav-item nav-link btn-success <%=array[1] %>" href="/studentdata/">基本信息管理</a>
		    <a class="nav-item nav-link btn-success <%=array[4] %>" href="/selfinfo/">个人信息管理</a>
		    </c:if>
		    <c:if test="${cookie['identity'].value eq 'teacher'}">
		    <a class="nav-item nav-link btn-success <%=array[1] %>" href="/teacherdata/">基本信息管理</a>
		    <a class="nav-item nav-link btn-success <%=array[4] %>" href="/selfinfo/">个人信息管理</a>
		    </c:if>
		</nav>
